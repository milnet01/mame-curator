...
| Field | Value |
|-------|-------|
| **Project phase** | P04 — HTTP API (in progress; blocked on FP09 fix-pass) |
| **Active item ID** | FP09 |
| **Active step** | 1 — verify spec (skipped per "specs are for features not fixes" rule) |
| **Blocked on** | — |
| **Last update** | 2026-05-01 (P04 Step 4 shipped — 363 tests pass / coverage 88.49% / all five gates green; close attempted via `/close-phase`; `/audit` clean; `/indie-review` surfaced 13 actionable findings folded into **FP09** generated by /close-phase; P04 stays 🚧 until FP09 closes) |
| **Next gate** | FP09 9-step loop → re-run `/close-phase` on P04 |
...
FP09 active — step progress (reset by /close-phase 2026-05-01):

1. ⬜ Verify spec — skip per "specs are for features not fixes" rule; the per-bullet contract lives in `ROADMAP.md` § FP09.
2. ⬜ Verify dependencies — P04 ship complete; FP09 builds on it.
3. ⬜ Write failing tests (per finding — A1 control-byte error tests, A2 corrupt-report.json fixture, A3 reserved-name import test, B1 default-equal-Session round-trip, B5 timing harmonisation, B6 `replace_world` identity preservation, B9 fsync test, C1 subscribe-after-start replay, etc.).
4. ⬜ Implement until tests pass
5. ⬜ Run `/audit`
6. ⬜ Run `/indie-review`
7. ⬜ Fold any actionable findings → new FP## (or close FP09 cleanly + close P04 in same `/close-phase` re-run if both are clean).
8. ⬜ Update CHANGELOG / ROADMAP / journal
9. ⬜ Commit, tag `FP09-complete`, ask user about push

### Active item details

FP09 — P04 indie-review fold-in. 13 actionable items: 3 Tier-1 (A1 `{exc}` repr-quoting at 5 sites; A2 R27 typed `CopyReport` response; A3 R19 session-name regex re-enforcement) + 9 Tier-2 hardening (B1-B9: round-trip symmetry, listxml triple-pass, unbounded `Job.history`, per-request `httpx.AsyncClient`, pause/resume timing harmonisation, `replace_world` identity preservation, `fs_list` parent dead-branch + sandbox filter, `_help_dir()` arithmetic, parent-dir fsync) + 3 Cluster-R doc/test (C1 subscribe-after-start race test; C2 `quote()` deferral comment; C3 R20 dry-run vs playlist-conflict spec line). Source: `/indie-review` 2026-05-01 against the P04 surface. Closes when 9-step loop returns clean to `/close-phase`; P04 then closes in the same run.
...
| P04 | Phase 4 | 🚧 | — | — | HTTP API (`api/`) — implementation shipped 2026-05-01; close blocked on FP09 |
| FP09 | — | 🚧 | — | — | P04 indie-review fold-in (13 actionable: 3 Tier-1 / 9 Tier-2 / 3 Cluster-R) |
