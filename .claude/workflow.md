...
| **Active item ID** | **none pinned** — user to pick the next media/ item. Queue: 1084 (Settings enable/disable), 1079 (MobyGames cover-fetch — **blocked on a real MobyGames API key fixture**). (**1081 + 1082 CLOSED 2026-07-03**.) |
...
| **Last update** | 2026-07-03 — **mame-curator-1081 CLOSED** (✅, implement). Added `media.snaps_dir` config field binding the progettoSnaps read-path to `refresh-snaps --dest`: `api/routes/media.py` passes `config.media.snaps_dir/"snap"` into build_registry/build_all_sources; `refresh-snaps` gained `--config` and reads `media.snaps_dir` when `--dest` omitted; Settings → Media "Snapshot pack folder" field + type-sync + spec/example-config docs. TDD (failing API + 4 CLI tests first). Full gate green (backend 861, cov 88.5%; frontend tsc/eslint/344 vitest; api-types-sync; gitleaks). Prior: **mame-curator-1082 CLOSED** — added `httpx2>=2.5` to the dev extra so Starlette 1.2's TestClient imports `httpx2 as httpx` and no longer emits a `StarletteDeprecationWarning` on every `pytest` run (migration, not a filter). Full backend gate green (856 passed, cov 88.12%), pushed, **CI green**. Also reconciled the Ants MCP feedback file: verified 4 previously-📋 items (ANTS-2054/3415/3416/3417) shipped live (build cb468d5) and logged via `feedback_log`. Prior: **mame-curator-1058 CLOSED** — authored `src/mame_curator/media/spec.md` (co-located contract for the shipped `media/` package), distilled from P05/P10 and verified clause-by-clause against `src/mame_curator/media/*.py`; ran **5 `/cold-eyes` loops** to convergence (HIGH→MEDIUM→LOW, 6 cold readers), all verified findings fixed inline. Also (session, non-1058): added `local-CI.sh` mirroring `ci.yml` (ae5889d) and `coding-standards.md §8` latest-by-default dep rule + version-break registry (6b670bd), both pushed + CI-green. |
| **Next gate** | User picks the next media/ item (suggest 1081 — small + unblocked). 1079 stays blocked until a MobyGames key is available. |
