...
| Field | Value |
|-------|-------|
| **Project phase** | P05 — Media subsystem (planned) |
| **Active item ID** | P05 |
...
9. ⬜ Commit, tag `P05-complete`, ask user about push

### Active item details

P05 — `media/` module. Long-form contract: `docs/superpowers/specs/2026-04-27-roadmap.md` § Phase 5. Coverage target: ≥90%. URL escape rules (`&*/:\<>?\|"` → `_`); `urls_for(machine)`; async `fetch_with_cache(url, cache_dir)`; cache key = `sha256(url)`. P04's R39 minimal proxy already exposes the route; P05 swaps in proper escape rules + sha256-keyed disk cache by replacing the handler body — same URL surface, no client breakage.
...
| P05 | Phase 5 | 🚧 | — | — | Media subsystem (`media/`) — next |
