...
| **Active item ID** | **none pinned** — user to pick the next item. Media/ queue: 1079 (MobyGames cover-fetch — **blocked on a real MobyGames API key fixture**). (**1076 + 1084 CLOSED 2026-07-04**; 1081 + 1082 + 1058 CLOSED 2026-07-03.) Other open queues: cleanup-debt (1075 Prettier), test-audit (1040 conftest fixture scope), cold-eyes (1062 Radix-Esc lock, 1063 docs/help build gap), features (1038 auto-save, 1039 P16 UI polish), community-funding (1073 Sponsors link). |
| **Active step** | — (between items). **2026-08-03: mame-curator-1088 CLOSED** ($PORT contract on both serve entry points; cold-eyes fixed a rich-markup swallow in its own error message; CI red on windows-latest fixed + a local guard for the class; `local-CI.sh` now runs as a pre-push hook, doc-only pushes exempt). Doc-layout migration landed — `docs/superpowers/` is gone, `docs/design.md` + `docs/plans/` + the `spec-format` trio are in place. New queue: 1089 (run.bat $PORT gap, needs a Windows runner), 1090 (--port/--host help drift), 1091 (nine pre-existing cli/spec.md defects), 1092 (rename 16 legacy spec files). |
