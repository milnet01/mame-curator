...
| **Active item ID** | P14 |
| **Active step** | 3/9 ⬜ — tests-first. |
| **Blocked on** | nothing |
| **Last update** | 2026-05-17 (P14 Step 2 closed: implementation plan inlined into `docs/specs/P14.md` as "Implementation plan (Step 2)" section per FP05/07/08 precedent. 16 commit-sized chunks sequenced: chunks 1–3 backend foundation (ReviewState model + tagged-union ActivityEvent extension + WorldState/replace_world passive swap), chunks 4–5 API surface (three /api/state routes + per-request games filter), chunk 6 INV-12 parity contract test, chunks 7–8 frontend foundation (types + useReviewState hook + strings + activity renderer), chunk 9 useGameGridFocus refactor (absorbs LibraryGrid's FP21-T roving-tabindex — must precede 10/12/14), chunks 10–14 feature wiring (badge, segmented filter, grid R/S/? + walkthrough, header chip + WalkthroughToggle, drawer R/S/?), chunk 15 Playwright e2e round-trip, chunk 16 spec-promotion + tag `P14-complete`. Known sequencing constraint: chunks 4 + 7 must land back-to-back in one push (or merge into one commit) because the bidirectional type-sync pre-commit gate fails any commit that adds Pydantic types without TS mirrors. Plan acknowledges LibraryPage.tsx pre-existing hard-cap breach as not-a-blocker. Coverage check before chunk 16 gates: filter/review_state ≥ 95%, new curate routes ≥ 80%, frontend hooks+component surface ≥ 70%.) |
| **Next gate** | P14 Step 3 — tests-first. Land the failing tests listed under §"Tests to write first" in `docs/specs/P14.md`, sequenced per chunk: chunk 1 brings up `tests/filter/test_review_state.py` (6 tests RED); chunk 2 a copy types round-trip; chunk 3 `test_replace_world_review_state_param_passes_through_field`; chunk 4 the eleven `tests/api/test_state_routes.py` tests + integration round-trip; chunk 5 the two `test_review_state_query_param_*` tests + stage-order assertion; chunk 6 the INV-12 parity test; chunks 7–14 the frontend test surface (`useReviewState`, `useGameGridFocus`, `GameCard`, `FiltersSidebar`, `LibraryPage`, `AlternativesDrawer`). Each chunk's tests land in the same commit as its implementation (TDD-default per CLAUDE.md). |
