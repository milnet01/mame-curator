...
| **Active item ID** | P14 |
| **Active step** | 2/9 ⬜ — plan. |
| **Blocked on** | nothing |
| **Last update** | 2026-05-17 (P14 Step 1 closed: spec at `docs/specs/P14.md` (~720 lines). Cold-eyes review converged at loop 5 — 69 findings across loops 1-4 verified+fixed (7 CRITICAL, 21 HIGH, 22 MEDIUM, 19 LOW), final pass clean. Major corrections caught up front: (1) `replace_world(review_state=)` is a passive swap, no filter recompute — review state doesn't gate eligibility; (2) `?review_state=` per-request filter lives in `api/routes/games.py`, NOT in `run_filter()`; (3) `AlternativesDrawer` is shadcn `Sheet`, not bare Dialog; `ToggleGroup` not in project — uses `RadioGroup`; (4) `LibraryGrid` already owns focus via FP21-T — `useGameGridFocus` absorbs it, R/S/? lives in grid's existing React `onKeyDown`; (5) `ActivityEvent` extended via new `REVIEW_STATE` enum + `ReviewStateDetails` payload; (6) `strings_internal.ts` is the catalogue, not the `strings.ts` facade. 13 INVs codified, optimistic UI cache key `['reviewState']`, sparse-store with pending=absence, single file `data/state.yaml`, segmented `RadioGroup` filter, per-clone state in drawer, walkthrough auto-advance default-on. Gitignore: `.directory` + `/test-results/` ignored. User signed off after loop-5 convergence; ready for Step 2 (plan).) |
| **Next gate** | P14 Step 2 — implementation plan. Per FP05/07/08 precedent the plan is inlined in the spec body (`docs/specs/P14.md` gains an "Implementation plan" section sequencing the work: schemas + persistence module first, then API routes, then per-request filter, then frontend hook refactor + grid wiring + drawer wiring + header chip + strings). |
