...
| Field | Value |
|-------|-------|
| **Project phase** | P14 closed 2026-05-17 — per-game review state. Docs bundle (mame-curator-1029 + 1030) closed 2026-05-16. DS03 closed 2026-05-16. DS05 closed 2026-05-16. DS02 closed 2026-05-15 + R2 hot-fix. FP28 closed 2026-05-15. DS04 closed 2026-05-15. FP27 closed 2026-05-14. Queue continues **post-v1**. |
| **Active item ID** | (none) |
| **Active step** | (none — between phases) |
| **Blocked on** | nothing |
| **Last update** | 2026-05-17 (P14 closed; tag `P14-complete` annotated. Backend: `filter/review_state.py` (frozen Pydantic + 2 enums + loader), `api/routes/curate.py` (3 routes: GET/POST/DELETE /api/state with `_persist_review_state` + `_build_review_event` helpers), per-request `?review_state=` filter on `GET /api/games` (NOT in `run_filter()`; sits after the existing `keep()` slice), `WorldState.review_state` + `replace_world` kwarg (INV-4 passive swap), `ActivityEvent` extended with `REVIEW_STATE` arm + `ReviewStateDetails` payload, INV-12 parity contract test. Frontend: `useReviewState` (optimistic onMutate/onError/onSettled, cache key `['reviewState']`), `useGameGridFocus` extracted from `LibraryGrid` (absorbs FP21-T + adds `focusNextPending(startIndex)` and `setActive(idx)`), `GameCard` parallel review-state badge maps, `FiltersSidebar` segmented `RadioGroup` filter, `LibraryGrid` R/S/? + walkthrough auto-advance, `AlternativesDrawer` R/S/? + ↑/↓ via `useKeyboard` (INV-7 — no auto-advance from drawer), `LibraryPage` header progress chip + walkthrough toggle persisted to `localStorage`. Shipped across 11 commits on `main`: 87ba106 chunk 1, 4eedaae chunk 2, e70af4f chunk 3, 4794dea chunks 4+7 combined (type-sync gate), f4df2bd chunk 5, 43152b1 chunk 6, a2c5abc chunk 9, b53ad7b chunk 10, e245f12 chunks 11+12+13, cf85ead chunk 14, + close commit. DS05 count pins: pytest 507→535 (+28), vitest 289→300 (+11). 13 INVs codified in `docs/specs/P14.md` enforced by tests. Chunk 8 (planned dedicated activity-renderer switch case) collapsed into the generic-summary path — `ActivityPage` already displays the human-readable summary the chunk-4 emitter writes. Chunk 15 (Playwright e2e) deferred to a follow-up; per-test coverage in vitest + pytest pins behaviour against drift. All 5 backend gates green throughout; all 3 frontend gates green throughout.) |
| **Next gate** | (none — between phases; queue continues post-v1 per ROADMAP). |
