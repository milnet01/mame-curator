...
| Field | Value |
|-------|-------|
| **Project phase** | FP13 — FP12 closing-review fold-in (🚧 active; FP12 stays 🚧 until FP13 closes) |
| **Active item ID** | FP13 |
| **Active step** | ⬜ all (FP13 just generated by /close-phase against FP12; closing audit found 22 actionable findings + 1 deferred FP) |
| **Blocked on** | — |
| **Last update** | 2026-05-04 (FP13 generated by /close-phase. FP12 closing /audit was clean across the FP12 surface — ruff / ruff format / mypy / bandit / eslint / gitleaks / trivy / semgrep returned 0 findings on the FP12 lane (semgrep flagged dangerouslySetInnerHTML on HelpPage:67 — out-of-scope, P06 surface, deferred to next debt-sweep, not allowlisted). 3-lane /indie-review across primitives / FsBrowser / SettingsPage wiring returned 22 actionable findings — cross-cutting theme: silent react-query mutation errors flagged by 2 of 3 lanes (grant + patch + restore fire-and-forget without onError). Findings batched into 5 thematic clusters A-E (4 mutation-observability fixes / 3 destructive-confirm correctness / 7 FsBrowser correctness+UX / 4 a11y / 6 standards compliance). SettingsPage 551 lines vs §2 hard-cap 350 is the largest individual finding — extracts FiltersTab / PickerTab / UpdatesTab / MediaTab per existing SnapshotsTab/BackupTab shape. FP12 step 4 implement is complete; FP12 stays 🚧 pending FP13 close.) |
| **Next gate** | Work FP13 through the 9-step loop. Step 1 (verify spec) — FP13 is a fix-pass; the audit surface IS the FP13 ROADMAP entry's 22 sub-bullets (per "specs are for features, not fixes"). Step 2 — depends on FP12 (still 🚧, by design). Steps 3+4 — TDD per cluster, one commit per cluster per FP11 cadence. Closing strategy: /close-phase. Once FP13 closes clean, FP12 can close at the same SHA per the P05+FP10 / P06+FP11 precedent. |
...
| FP12 | — | 🚧 | — | — | Settings page list editors + path picker (active 2026-05-02; impl complete 2026-05-04; awaiting FP13 close) |
| FP13 | — | 🚧 | — | — | FP12 closing-review fold-in (active 2026-05-04; 22 findings across 5 clusters A-E; depends on FP12 🚧) |
