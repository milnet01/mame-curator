...
| **Active item ID** | **mame-curator-1095** (package; lanes ci/cli/packaging) — self-contained desktop bundles. Spec `docs/specs/mame-curator-1095-desktop-bundles.md` (cold-eyes stopped at loop 2 on the collateral trigger — do NOT re-run it), build order `docs/plans/mame-curator-1095-desktop-bundles.md` (16 steps). Queued behind it — media/: 1079 (MobyGames cover-fetch — **blocked on a real MobyGames API key fixture**). Serve/CLI queue after the 1090 close: **1089** (run.bat: unconditional `--port` skips the `$PORT` check *and* makes `server.port` unreachable on Windows — needs a Windows runner), **1092** (rename 16 legacy `docs/specs/<ID>.md`, ~207 citations), **1094** (13 deferred cli/spec.md doc findings). Other open queues: cleanup-debt (1075 Prettier), test-audit (1040 conftest fixture scope), cold-eyes (1062 Radix-Esc lock, 1063 docs/help build gap), features (1038 auto-save, 1039 P16 UI polish), community-funding (1073 Sponsors link). |
| **Active step** | **1095 plan steps 1–5 done, next is a SPEC AMENDMENT (fold-in), then step 6.** Paused 2026-08-04 mid-`/write-spec`: the amendment was researched and briefed but **not a byte was written** — the brief is the ROADMAP progress note dated 2026-08-04 ("Fold-in pending"), and it must be drafted, `/doc-lint`ed and `/cold-eyes`-gated before the packaging steps (8–15) are built, because it changes what they build. Step 4 (41637cc): `build_world` catches `(ParserError, OSError)` → empty library + `WorldState.setup_required=True` (INV-7/8); `replace_world` passes the flag through (found by implementing, folded into §4.2). Step 5 (5bda03a): `_validate_paths(..., setup_required=)` skips the `source_dat` check and `restart_required = server_changed or world.setup_required` (INV-9/10). Step 6 next: `setup_required` on `SetupCheck` + the TS mirror in the SAME commit or `check_api_types_sync.py` reds CI. **CI caught a Windows-only failure** the local mirror cannot see (7e3ad7a): `re.subn` expands backslash escapes in an f-string replacement, so a `D:\a\...` tmp_path raised `re.error: bad escape \m` — use a callable replacement. Earlier: steps 1–2 (2026-08-04, commit 9842a86): `serve` resolves `config.yaml` through `--config` → `./config.yaml` → per-user dir and writes a starter config + data dirs on first run (`src/mame_curator/config_location.py`). Step 3 (this session): `src/mame_curator/_resources.py` — `bundle_root()` / `frontend_dist()`; `create_app` resolves the SPA path at construction instead of import (INV-11). Step 4 is `build_world` degrading to `setup_required` on an unreadable DAT. Steps 8–16 are the packaging half; two of the three local build scripts can run here (Windows under Wine, macOS cannot — INV-16). **2026-08-04: mame-curator-1090 + 1091 CLOSED** — the `serve` config layer, TDD, spec-gated first. **2026-08-03: mame-curator-1088 CLOSED** ($PORT contract on both serve entry points; cold-eyes fixed a rich-markup swallow in its own error message; CI red on windows-latest fixed + a local guard for the class; `local-CI.sh` now runs as a pre-push hook, doc-only pushes exempt). Doc-layout migration landed — `docs/superpowers/` is gone, `docs/design.md` + `docs/plans/` + the `spec-format` trio are in place. New queue: 1089 (run.bat $PORT gap, needs a Windows runner), 1090 (--port/--host help drift), 1091 (nine pre-existing cli/spec.md defects), 1092 (rename 16 legacy spec files). |
...
| **Last update** | 2026-08-04 (evening) — **1095 steps 4 + 5 shipped, CI green at 7e3ad7a**; the first-run recovery journey now works end to end (start on a bad DAT → save the fix → prompted to restart). Also **mame-curator-1096 CLOSED** (✅) — stray browser tabs traced to `frontend/screenshots/playwright.config.ts` starting `serve` against the REAL `config.yaml` (`open_browser_on_start: true`) with no `--no-open-browser`; the ROADMAP bullet records a *wrong* first diagnosis (blamed the CLI test suite) alongside the right one, deliberately, because the wrong one was reported to the user. **Paused for the night mid-fold-in** — see the Active step row. Prior: **mame-curator-1090 + 1091 CLOSED** (✅). `serve` now resolves host/port/browser through `config.yaml`'s `server:` block: `--port` → `$PORT` → `server.port` → 8080, `--host` → `server.host` → `127.0.0.1`, and the browser opens from a daemon thread that polls the socket instead of `run.sh`'s blind 2 s sleep. Only the `server:` block is parsed, never the whole `AppConfig`. Also closed 1091's code half — `--no-open-browser` is read, `OverflowError` is caught at the bind, and all six `_cmd_serve` error messages escape rich markup. `run.sh` took three changes (drop `:-8080`, guard the validation on `[ -n ]`, conditional `--port`) plus the announce-line split; `run.bat` lost its browser open. Spec was written and cold-eyes-gated *before* the code (`cli/spec.md`, 3 loops, converged by cap); every `PENDING` marker in it is now swept. `docs/specs/P04.md` corrected reciprocally (exit 130, flag surface, ticked checkbox) — the two docs no longer contradict. TDD, +33 tests (947 total), full `local-CI.sh` green. Two commits: 2033bdd, 5933127. Prior (2026-07-04): **mame-curator-1084 CLOSED** (✅, feature, frontend). Settings → Media per-source on/off toggle: `MediaSourceRow` gained a `Switch` (checked=in_chain; onToggle add/removes from `media.sources` via PATCH /api/config); `MediaTab` renders unconfigured known sources (in_chain=false) in an "Available sources (off)" list below the DragReorderList; libretro's toggle is locked-on (registry always re-appends it via `MediaSourceRegistry.chain_for`). Frontend-only — the readiness endpoint already returned every source with an `in_chain` flag. TDD (+9 vitest first; user picked toggle-switch over checkbox). Full gate green (backend 861/88.5%; frontend tsc/eslint/**353** vitest; api-types-sync; gitleaks) — first CI push went red on the DS05 vitest-count guard (327→336), fixed in a6bf1b9, **CI green**. Also **mame-curator-1076 CLOSED** (✅, chore, ci) — verified `macos-latest` green on macOS 26 mid-rollout (CI run 28667637025: 3.12 leg on macos-26-arm64, 3.13 on macos-15, both passed); kept unpinned per latest-by-default, no workflow change. Logged an ANTS-3417 non-reproduction (roadmap-flip trailing whitespace — 2/2 clean commits) to the Ants MCP feedback file. Prior (2026-07-03): 1081 (media.snaps_dir), 1082 (httpx2 dev dep), 1058 (media/spec.md + 5 cold-eyes loops) all CLOSED + CI-green. |
| **Next gate** | User picks the next item. 1079 stays blocked until a MobyGames key is available; all other queues (cleanup-debt / test-audit / cold-eyes / features / funding) are startable. |
...
| **Repo visibility** | PUBLIC (cached 2026-04-30 via `gh repo view`) |

### Step progress

Tracks the CURRENT active item only; resets to all ⬜ when a new item
becomes active. Closed-phase step logs live in each phase's
