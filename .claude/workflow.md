...
| Field | Value |
|-------|-------|
| **Project phase** | P08 — Setup wizard (📋 next) |
| **Active item ID** | P08 |
...
| **Blocked on** | — |
| **Last update** | 2026-05-04 (P07 closed — slim version: downloads.py + refresh-inis CLI + cards_per_row Select + HelpPage DOMPurify + Cmd-K help wire-up. Earlier today: FP14 closed — GameCard layout overflow fix from user screenshot. Earlier today: FP13 + FP12 closed at the same SHA per P05+FP10 / P06+FP11 precedent. All 22 FP13 findings closed across 6 commits (one per cluster) — A mutation observability + restart_required banner, B destructive-confirm correctness, C FsBrowser correctness + UX, D accessibility WCAG 2.2, E1 SettingsPage extract under 350-line hard cap, E2-E6 hardening sweep. SettingsPage went from 551 → 305 lines via 6 new extractions: FiltersTab / PickerTab / UpdatesTab / MediaTab / PrefSwitch / PathRow. New helper `lib/apiErrorToast.ts` translates ApiError → strings.errors.byCode with detail fallback; wired through useConfigPatch / useSnapshotRestore / useFsGrantRoot. 177 frontend / 435 backend tests / lint+typecheck+ruff+mypy+bandit / build all clean at close. P11 added 2026-05-04 — "Contribute missing thumbnails upstream" (post-v1, depends on P05). Repo PUBLIC; pushed FP12-complete + FP13-complete tags after close.) |
| **Next gate** | P08 step 1 — verify spec. Long-form contract at `docs/superpowers/specs/2026-04-27-roadmap.md` § Phase 8. Setup wizard: `run.sh` / `run.bat` Stage 1 (Python + uv detection, port check, browser launch) + Stage 2 in-browser wizard (Welcome → Source paths → Destination → Reference data download (reuses P07 § A `downloads.py` and § B refresh-inis logic) → Filter preview → Done). Reuses `<FsBrowser>` from FP12 § G for path picking. Resumability across reboots via a small JSON state file. v1 critical path: P08 (this) → P09 (release polish + tag `v1.0.0`). |
