...
| **Active item ID** | FP34 (mame-curator-1087) **shipped 2026-07-02** (commit a4aac3e). P10 `/close-phase` 3rd attempt: audit clean (allowlist-015, now ledgered) + 2-lane verification confirmed all FP32/FP33 fixes correct; 3 tail findings → FP34, now done. **Parent P10 (mame-curator-1005) code is DONE + verified**; the only gate left before `P10-complete` is the co-located **`src/mame_curator/media/spec.md` (mame-curator-1058)** — a P05-era gap the module still carries. |
| **Active step** | Media surface fully hardened across FP32→FP34 (3 review rounds, severity trailed to one-liners) + CI-green. **Decision pending: author media/spec.md now (1058, with /cold-eyes) then tag P10-complete, OR tag P10-complete now + do 1058 as the immediate next item.** |
| **Blocked on** | user decision on the media/spec.md-vs-tag sequencing (see Active step). No code work outstanding on P10. |
| **Last update** | 2026-07-02 (**FP34 shipped — P10 3rd/final closing-review fold-in**. 2-lane verification confirmed FP32+FP33 fixes all correct; 3 tail findings burned off: **M1** MobyGamesSource `_resolve_key` read moved inside the OSError guard (now catches `UnicodeDecodeError` too → self-disable on a corrupt key file, was a 500-every-request vector); **M2** dropped a spurious `Mut. ✓` on the secret route in api/spec.md; **L1** added `byCode.media_source_unknown`. Committed `.ants_review_falsepos.jsonl` (machine-readable allowlist-015 mypy ledger, via `audit_falsepos_log`). Gates: 855 backend @88% / 342 frontend vitest, all clean; DS05 pytest 685→686. Convergence: 3 FP in a row (FP32/33/34), under 5. **P10 code done; awaiting the media/spec.md-vs-tag call.** **Prior:** 2026-07-02 (**FP33 shipped** — the P10 second closing-review fold-in. Reproduce-before-fix per behavioural finding. **H1** (SECURITY/LFI): `resolve_image`'s `file://` short-circuit gated to the local pack source by name — a network source's `file://` URL (ArcadeDB plaintext hop / MITM `file:///etc/passwd`) is now dropped, not read off disk; **H2** nested type guards added across ArcadeDB (`result`/`result[0]`/non-str URL fields), WikipediaImage (non-dict `thumbnail`), wiki-extract (non-dict `content_urls.desktop` — the last 500'd the never-500s wiki path); **M1** progettoSnaps probe wrapped in `except OSError` → self-disable (also collapsed the double `is_dir()`); **L1** DownloadPackModal resets `copied` on close; **L2** api/spec.md +3 shipped routes +`MediaSourceUnknownError` +corrected chain default order; **L3** stale `check_error_codes_sync.py` comment fixed; **L4** `SourceSecret` `.strip()` (whitespace-only → 422); **L5** secret-route single-source coupling documented; **INFO** ConfigureSourceKeyModal `onSuccess`→`handleOpenChange`, dropped retired `media_upstream_error` from P06.md. Gates: **854 backend @ 88%** / 0 ruff / 0 format / 0 mypy / 0 bandit; **342 frontend vitest** / 0 eslint / tsc + vite build clean; api-types-sync 67. DS05 pins 677→685 (pytest) / 324→325 (vitest). CHANGELOG `### FP33` subsection + ROADMAP flip ✅. **Next: re-run `/close-phase` on P10.** **Prior:** 2026-07-02 (**P10 `/close-phase` re-run → BLOCKED again; FP33 spawned** — `/audit` clean (28 warnings all allowlist-015). 3-lane `/indie-review` (media chain / API surface / frontend) confirmed FP32's 4 fixes correct but found **H1** LFI: `resolve_image` `file://` short-circuit trusts a URL from ANY source (ArcadeDB plaintext hop + unvalidated upstream strings) → `file:///etc/passwd` served as image, bypassing the cache scheme guard; **H2** FP32's `isinstance(data, dict)` only guards the top level — 4 nested accesses (ArcadeDB `result[0]`/non-str URL fields, WikipediaImage non-dict `thumbnail`, wiki non-dict `content_urls.desktop`) still 500; **M1** progettoSnaps still crashes on a permission-denied dir; **LOWs** api/spec.md missing 3 shipped routes + `MediaSourceUnknownError`, DownloadPackModal `copied` never resets, stale `check_error_codes_sync.py` comment, `SourceSecret` verbatim-store, secret-route name coupling. All folded into **FP33 [mame-curator-1086]** (review-fix; media/api/frontend/docs). Convergence: 2 FP in a row (under 5). **Next: work FP33 (TDD), then re-run `/close-phase`.** **Prior:** 2026-07-02 (**FP32 shipped** — the P10 closing-review fold-in. Reproduce-before-fix: a failing regression test landed first for each behavioural finding, then the fix. **H1** non-object JSON (`[]`/`null`/str) → `isinstance(data, dict)` parse-before-trust guard in ArcadeDB/WikipediaImage `prepare` (unlink slot + raise MediaFetchError → chain advances) + `resolve_wikipedia_extract` (unlink + return None); **H2** DownloadPackModal clipboard guard + resolve-only `copied` flip; **M1** progettoSnaps `.is_dir()` probe (FILE snap_dir self-disables, no NotADirectoryError); **M2** AboutSection `https://` scheme gate on the Wikipedia link; **LOWs** api/spec.md 502→404 fallback-chain drift, dead `MediaUpstreamError` class + byCode/strings_internal leaf removed, `TokenBucket._last` unconditional, resolve `file://` `.is_file()`, ConfigureSourceKeyModal surfaces `ApiError.detail` + defensive clear-on-close. Gates: **846 backend @ 88%** / 0 ruff / 0 format / 0 mypy / 0 bandit; **341 frontend vitest** / 0 eslint / tsc + vite build clean; api-types-sync 67. DS05 pins bumped 671→677 (pytest) / 319→324 (vitest). CHANGELOG `### FP32` subsection + ROADMAP flip ✅. **Next: re-run `/close-phase` on P10.** **Prior:** 2026-07-01 (**P10 `/close-phase` → BLOCKED; FP32 spawned** — all 11 chunks shipped + CI-green, so ran `/close-phase`. `/audit` (scope full): 0 errors, 29 warnings ALL the scrubbed-env `mypy` "cannot find fastapi/yaml" false positive (CI mypy clean) → logged **allowlist-015**; ruff/bandit/semgrep/gitleaks/shellcheck clean. 3-lane `/indie-review` (backend chain / API surface / frontend) surfaced **real defects the spec cold-eyes couldn't catch** (they reviewed contracts, not code paths): **H1** valid-but-non-object JSON (`[]`/`null`/str) → `.get()` AttributeError escapes parse-before-trust AND `resolve_image` → route 500 + persistent poisoned cache slot (ArcadeDB / Wikipedia-image / wiki-extract, no test); **H2** DownloadPackModal clipboard — no `navigator.clipboard` guard + false "Copied!" on failure; **M1** ProgettoSnapsSource `.iterdir()` on a FILE snap_dir → NotADirectoryError crashes registry; **M2** AboutSection renders Wikipedia `url` href with no `https:` scheme check; + LOWs (`api/spec.md` still documents the retired 502 surface; `MediaUpstreamError` now dead code; TokenBucket stale-anchor; file:// `.exists()` vs `.is_file()`; 422-detail discarded; secret cleared only via unmount). Per close-phase 5a: actionable>0 → folded ALL into **FP32 [mame-curator-1085]** (review-fix; media/api/frontend/docs lanes) in the Indie-review fold-in section; P10 stays 🚧. Convergence checkpoint: 1 FP in a row (well under 5). Also gitignored `.audit_cache/` (audit artifacts). **Next: work FP32 (TDD), then re-run `/close-phase`.** **Prior:** 2026-07-01 (**P10 chunk 11 — Wikipedia About paragraph (LAST chunk)** — `hooks/useWikipediaExtract.ts` (`useApiQuery(['wikipedia', name], '/media/{name}/wiki', WikipediaExtractSchema.nullable(), {staleTime: Infinity})` — nullable so a 200+null body → null, not a Zod throw) + `components/alternatives/AboutSection.tsx` (renders nothing while loading / on error / null; extract + read-more link + CC-BY-SA line on happy path), mounted in `AlternativesDrawer` under the header (`shortName={winner.short_name}`). 2 new strings (`alternatives.wikipediaReadMore` / `wikipediaLicense`). No spec amendment / cold-eyes (contract already complete from Step 1; nullable-schema is an impl detail the existing test pins). Test-infra: added a default MSW `GET /media/:name/wiki` → null handler; wrapped the `AlternativesDrawer` test's `renderWithRouter` in a QueryClient (AboutSection is a react-query consumer). 4 new vitest decls (hook null-body + AboutSection null/happy/loading; DS05 vitest pin 315→319). Gates: 336 frontend vitest / 0 eslint / tsc + vite build clean; backend untouched. **ALL 11 P10 chunks shipped — next is the phase close** (audit + indie-review, journal, spec move, tag P10-complete). **Prior:** 2026-07-01 (**P10 chunk 10 — Settings → Media source list** — extended the existing `MediaTab.tsx` (the tab + `DragReorderList` + chunk-9 types already existed). New `hooks/useMediaSources.ts` (`useMediaSources` = `useApiQuery(['media','sources'], …, SourceReadinessSchema, {staleTime:5000})` + `useSaveSourceSecret` = `apiRequestVoid` PUT → invalidate). New `MediaSourceRow` (status dot `data-state`, kinds, disabled_reason line, Configure button gated `needs_config && !enabled`, Download-pack button gated `progettoSnaps && !enabled`), `ConfigureSourceKeyModal` (password input → PUT → close+invalidate on 204, inline error on 422), `DownloadPackModal` (copy `mame-curator refresh-snaps` — terminal hint, **no backend route**). `DragReorderList` extended with an optional `renderItem` prop (label-only; non-breaking for PickerTab). MediaTab renders the reorderable readiness list (`media.sources` → `onChange('sources', …)` → PATCH /api/config) + the two modals. ~14 new strings keys. Spec amendment cold-eyes'd 2 loops (0 CRITICAL; fixed the stale `api.get`/`api.put` hook block → real client, `['mediaSources']`→`['media','sources']` key drift, chunk-table POST-endpoint fork; deferred enable/disable checkbox → **mame-curator-1084**). Test-infra: `_settingsPageFixtures` render helper + `no-checkbox-for-prefs` now wrap a QueryClient (MediaTab is a react-query consumer); added a default MSW `GET /api/media/sources` → `{sources:[]}` handler. Aliased strings (`const s = …`) reverted to full `strings.settings.X.Y` paths (the orphan-key checker can't trace aliases). 9 new vitest decls (MediaSourceRow 3 + ConfigureModal 3 + DownloadPack 1 + MediaTab 2; DS05 vitest pin 306→315). Gates: 332 frontend vitest / 0 eslint / tsc + vite build clean; backend untouched (DS05 pytest still 671). **Only chunk 11 (AboutSection) remains.** **Prior:** 2026-07-01 (**P10 chunk 9 — readiness API + secret write** — `GET /api/media/sources` (per-source readiness: enabled/in_chain/kinds-sorted-tuple/license_compatible/disabled_reason/needs_config; constructs all five via new `build_all_sources`, which shares a `_source_factories` table with `build_registry`) + `PUT /api/media/sources/{name}/secret` (writes `data/secrets/mobygames.key` via `atomic_write_text(mode=0o600)` — the `mode` kwarg is new, defaults 0o644; unknown-name → new `MediaSourceUnknownError` 422; empty-secret → `SourceSecret` min_length=1 422; value never logged; loopback-trust per item #5, user decision). New `mobygames_key_path` shared read/write path helper. New schemas `SourceReadinessRow`/`SourceReadiness`/`SourceSecret` + frontend mirror (`MediaKind` type + Zod) — type-sync 64→67 interfaces. `configured_media_sources` conftest fixture (default `[libretro]`, indirect-parametrized to all-5 for readiness tests). Spec amendment cold-eyes'd 2 loops (0 CRITICAL/HIGH after loop-1; resolved: readiness route needs app.state → `build_all_sources`; progettoSnaps-disabled-without-pack test contradiction split; `kinds` frozenset→sorted-tuple; `atomic_write_text` reuse w/ 0600; the loopback test omitted from the enumerated list). Item #5 resolved (loopback-trust); logged **mame-curator-1083** (app-wide CSRF hardening, considered). 14 new pytest decls (5 readiness + 9 secret; DS05 657→671). Gates: 840 backend @ 87.94% / 0 ruff / 0 format / 0 mypy / 0 bandit; 323 frontend vitest / 0 eslint / tsc + vite build clean; api-types-sync 67. **Prior:** 2026-07-01 (**P10 chunk 8 — Wikipedia extract endpoint** — user cleared the chunk-7 convergence checkpoint ("continue to chunk 8"). Shipped `media/wikipedia.py`: `WikipediaExtract` (frozen title/extract/url/license model) + `resolve_wikipedia_extract(machine, *, cache_dir, client, limiter)` — reuses the chunk-5 `_canonicalise_wikipedia_title` + REST-summary base, shares the `wikipedia_limiter` bucket AND the `fetch_text_with_cache` slot with `WikipediaImageSource` (same URL → same key → one lookup warms both), parse-before-trust unlink on bad JSON, returns None on 404 / missing-fields. New route `GET /media/{name}/wiki` registered **before** `/media/{name}/{kind}` (else `wiki` binds `kind`); catches `MediaError` → `null` (non-essential About text never 500s on rate-limit/network). Spec amendment cold-eyes'd 2 loops (0 CRITICAL/HIGH; resolved the `limiter`-injection gap — spec signature omitted it while the rate-limit test needed it — and the rate-limit-→-null route contract; fixed a `GameNotFoundError(...)` stub + count drifts). `WikipediaExtract` added to `PYTHON_SOURCES` in the api-types-sync gate + mirrored in `types.ts` + Zod `schemas.ts` (frontend consumer is chunk 11). 12 new pytest decls (8 wikipedia incl. 2 error-branch + 4 route; DS05 pin 645→657). Gates: 826 backend @ 87.83% / 0 ruff / 0 format / 0 mypy / 0 bandit; 323 frontend vitest / 0 eslint / tsc + vite build clean; api-types-sync 64 interfaces. **Prior:** 2026-07-01 (**P10 chunk 7 — registry + orchestrator + route wiring** — the media source-chain converges. Shipped `MediaSourceRegistry` (`media/sources.py`; per-request pure filter/orderer: unknown-name drop with process-wide-deduped WARNING, libretro baseline append, kind filter, disabled-source filter) + NEW `media/resolve.py` (`resolve_image` orchestrator — walks the chain, swallows per-source `MediaRateLimited`/`MediaFetchError` and falls through, `file://` short-circuit for the local snap pack via `url2pathname` for Windows-correctness; + `build_registry` composition root injecting the app-state limiters + `SourceDisabledFlag`, constructing only configured sources so dropping mobyGames suppresses its keyless WARNING). Route `GET /media/{name}/{kind}` swapped inline URL build → `resolve_image`; **502 `media_upstream_error` retired for media** (single-source 5xx/transport error now falls through → 404 when the whole chain misses); dropped the `MediaUpstreamError` import. New `media.sources` config field (default 5-source order) mirrored in `types.ts` + Zod `schemas.ts` + 2 frontend fixtures. Chunk-7 also added a process-wide dedup guard to the keyless-MobyGames WARNING (per-request reconstruction would otherwise spam) and fixed two shipped MobyGames tests filtering the wrong caplog logger (`media.sources`→`media.mobygames`). **Spec amendment cold-eyes'd 5 loops to a clean, implementable contract** (0 CRITICAL throughout; fixed stale `_enabled`-flag prose across 4 sites, bare-string-vs-`SourceDisabledFlag` holder drift, injectable `secrets_dir` + POSIX-only mode-gate docs, `{"release":[]}`→`{"release":0,"result":[]}` test shape, false "shipped" heading, registry `available`-map contract gap, snap_dir phrasing, 502→404 tense, line-count/logger-name accuracy). 17 new pytest decls (registry 7 + resolve 9 + mobygames dedup 1; DS05 pin 628→645). Gates: 814 backend @ 87.78% / 0 ruff / 0 format / 0 mypy / 0 bandit; 323 frontend vitest / 0 eslint / tsc + vite build clean; api-types-sync ok. Route tests: the 3 P05-era 502-asserting tests rewritten to fall-through-then-404; `config_file` fixture pinned `media.sources: [libretro]` for deterministic single-source route tests (multi-source fall-through unit-tested in `test_resolve.py`). Follow-ups logged: **mame-curator-1081** (`media.snaps_dir` field binding source read-path to `refresh-snaps --dest`), **1082** (Starlette `httpx2` deprecation warning). **PAUSED at the convergence checkpoint** — user check-in due before chunks 8–11. **Prior:** 2026-07-01 (**P10 chunk 6 — MobyGames key-handling half** — user elected "key-handling now, fetch later" at the chunk-6 gate, since the cover-image fetch needs a real MobyGames API key to verify the response field path (none on this machine; no anonymous API access; the spec's `cover_url` guess is unverified). Shipped: `MobyGamesSource` in new `src/mame_curator/media/mobygames.py` (env-var / mode-0600-dotfile key resolution, missing-key + 401/403 disable, 429/404 auth branches, key-redacted errors, rate-limit) + `SourceDisabledFlag` holder (refines the spec's bare-`app.state` string — `media/` is HTTP-agnostic so the process-wide reason rides an injected holder). Deliberate divergence: `prepare` does its own `client.get` (not `fetch_text_with_cache`) to tell 401 from 429 apart AND keep the keyed URL out of logs. Extracted to its own module because `sources.py` + MobyGames = 596 > 500 hard cap. Schema field `media.mobygames_rate_limit_per_min` (default 5) + `mobygames_limiter`/`mobygames_disabled` on `app.state`. 17 new pytest decls (DS05 pin 613→628). Frontend: `MediaConfig` field mirrored in `types.ts` + Zod `schemas.ts` + 2 test fixtures (tsc/eslint/vitest 323/build green). **Cover-URL parse + JSON-body caching DEFERRED to mame-curator-1079** (post-P10 fix-pass; MobyGames is last in the fallback order so zero user-visible regression). Spec amended inline (chunk-6 notes); `/cold-eyes` re-loop on the divergences deferred to the chunk-7 convergence checkpoint per the spec's risk-register clause. Gates: 797 backend @ 87.73% / 0 ruff / 0 format / 0 mypy / 0 bandit; api-types-sync ok. **Prior:** 2026-06-30 (**Cleanup/debt bundle** — closed mame-curator-1077 (split `LibraryPage.tsx` 579→314 under the §2 frontend component hard cap of 350; non-render logic → new `pages/useLibraryController.ts` 318-line hook + pure `pages/libraryPageHelpers.ts` 61; JSX kept in the page so the `?raw` source-text structural tests stay valid; behaviour-preserving) + 1078 (Snapshots review-state exclusion caveat — `snapshotsStateExclusionNote` string per P14 spec §875, rendered in both empty + populated states; +2 vitest). Same `frontend` lane, both surfaced 2026-06-30 from the P14 docs work. 1076 (chore/ci macOS-26 pin) left for its own pass. 2 commits `3b0321b..d82c491` — **CI went red on the first push** (`test_ds05_test_count_stable` is a *pytest* test counting frontend `it()` decls; +2 SnapshotsTab tests tripped 304→306; local frontend gate doesn't run pytest), fixed by `d82c491` bumping the pin; final CI all 8 jobs green. 323 frontend vitest / 0 eslint / `tsc -b` + `vite build` clean; no other `.py` touched. Active P10 item unchanged.) **Prior:** 2026-06-30 (**P14 docs bundle** — closed mame-curator-1060 (P14 phase journal) + 1061 (review-state contract promoted to co-located `src/mame_curator/filter/review_state_spec.md`; user elected the separate-file option; `/cold-eyes` clean at 3 loops — fixed a world-lock over-claim, an unshipped-caption claim, a GET-can't-404 nit, and a coupled `copy/spec.md` `ReviewStateDetails` `str`-not-enum type bug) + 1064 (amended the CLAUDE.md "fix-passes don't get specs" rule to bless long-form specs for multi-tier fold-ins, matching the FP05/FP27/FP28/DS01–DS05 reality). **Parked 1058** (media spec — P10 still in flight, chunk 6+ unshipped; re-checked + annotated). Surfaced + roadmapped **1077** (`LibraryPage.tsx` 579-line file-size cap breach) and **1078** (unshipped Snapshots "review state not snapshotted" caption). 3 commits `2037993..cb6d8c7`; docs gate 96 green (no `.py`/frontend touched); pushed. Active P10 item unchanged.) **Prior:** 2026-06-10 (**FP31 second-sweep backend-test refactor bundle** — closed mame-curator-1046/1050/1051/1054/1055/1056 (same kind+lane: refactor / backend-tests). 1046 split `test_sources.py` 554→4 per-source files + `tests/media/conftest.py`, and `test_fp01_fixes.py` 423→272 + new `test_fp01_error_branches.py`; 1050 dropped 15 redundant `@pytest.mark.asyncio`; 1051 pinned 5 `pytest.raises` to `ValidationError` + `match=`; 1054 (a/c/d/e/f) helper/conftest dedup — **(b) deliberately declined** (cross-tree fixture move would collide `source_dir`/`dest_dir` names); 1055 all 15 sub-items a–o (incl. +1 X_OK test, makeGameCard factory, preflight two-run delta); 1056 `asyncio.run`→`async def`. Gates green: **770 backend** @ 87.40% / 0 ruff / 0 format / 0 mypy / 0 bandit; **320 frontend** vitest / 0 eslint / `tsc -b` + `vite build` clean; api-types-sync ok. Net +1 backend test (1055b); all else count-neutral. Open FP31 remainder: 1047/1048/1049/1052/1053.) **Prior:** 2026-06-10 (**test-audit 2026-05-20 coverage/reliability bundle** — closed mame-curator-1067 (flag-disabled `=False` keeps-them locks for the 4 remaining togglable drop predicates + explicit zip-slip test), 1068 (deterministic B7 allowlist-parent assertion + catch-all-respx no-HTTP proof for the two `prepare_is_noop` tests), 1069 (7 read-only api/conftest Path fixtures → `scope="session"`), 1070 (prose `# noqa: E402` reworded — `ruff check --no-cache` clean). +2 pytest decls (DS05 pin 608→610); 769 backend green @ 87.51%; ruff/format/mypy/bandit clean. Zip-slip + no-HTTP locks mutation-verified.) **Prior:** 2026-05-20 (**test-audit sweep** — full `/test-audit` across all 18 dimensions / all severities / 167 files (113 backend + 54 frontend = 50 vitest + 4 Playwright) → ~33 raw findings → 14 fixed inline (all count-neutral: 5 backend assertion/correctness, 2 slow-marker, 1 parametrize, 5 frontend correctness/reliability, + **1 pre-existing red test root-caused** — `test_arch_diagrams.py` lacked the `frontend/` exception its README sibling carried; `main` was red at HEAD `7ff03ec` from the 2026-05-18 cold-eyes docs sweep). 6 deferred clusters mame-curator-1065..1070; many lower nits re-confirmed already-open FP31 items (1046/1048/1050/1052/1054c/1055a,b,i) and were not re-filed. No new allowlist entry — the `@pytest.mark.asyncio` "FP" turned out to be the existing mame-curator-1050 cleanup. Test-count pin unchanged (611 backend / 305 frontend — every fix count-neutral). Gates green: 761 backend / 0 ruff / 0 mypy / 0 bandit; 320 frontend vitest / 0 eslint. Pre-existing `tsc` errors in `src/test/handlers.ts` + `renderWithClient.tsx` left untouched.) |
| **Next gate** | **Re-run `/close-phase` on P10** (mame-curator-1005, 3rd attempt) — audit + indie-review over the P10+FP32+FP33 surface. Clean → author `src/mame_curator/media/spec.md` (unblocks mame-curator-1058), move spec, tag `P10-complete`, flip P10 ✅. Findings → next FP. MobyGames cover-fetch stays deferred to mame-curator-1079; enable/disable to 1084. |
