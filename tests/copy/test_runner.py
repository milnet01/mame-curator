"""End-to-end tests for `run_copy` orchestrator — dry-run + apply + playlist conflict.

DS05 Cluster B: pause/resume/cancel + DS01 + FP05 tests extracted
to `test_runner_lifecycle.py` so the file stays under the 500-line
hard cap. The `_machine` / `_plan` factory helpers moved to
`_runner_helpers.py` (sibling, leading-underscore = not a test file)
so both this file and the lifecycle file import the same factories.
"""
