...
- 📋 [mame-curator-1029] **README hero shot + 4–6 screenshots.**
  Add a header image and a screenshot grid (library page, copy
  flow, settings tabs, sessions panel) under a new `## Screenshots`
  section. Replaces the all-words README that ships today.
  Layman: Right now the README is a list of words. Add screenshots
  so someone landing on the GitHub page can see what the app looks
  like before they decide to install it.
  Kind: doc.
  Lanes: docs.
  Source: planned (deferred from P09 slim, 2026-05-04).

- 📋 [mame-curator-1030] **CONTRIBUTING.md.**
  A short guide for contributors: how to run the local CI gate
  (`uv run pytest && uv run ruff check && uv run mypy …`), the
  TDD-first convention (tests fail before code that makes them
  pass), the App-Build 9-step phase loop, and the per-module
  `spec.md` requirement.
  Layman: A guide for people who want to fix bugs or add features.
  Explains the project's test-first workflow and how to run the
