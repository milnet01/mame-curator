...
- Phase-closing commits: scope `(parser)` / `(filter)` / etc.
  with the phase name in the subject:
  `feat(filter): phase-2 rule chain with deterministic curated output`
- Fix-pass commits: cite `pass-N TierM` in the subject:
  `fix(filter): pass-3 Tier 1 — wire drop_bios_devices_mechanical`
