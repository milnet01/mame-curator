...
- (b) Hoist both to
  `frontend/src/pages/__tests__/_settingsPageFixtures.ts`
  (leading underscore = "not a test file, helper only"),
...
Write **three structural-assertion tests** that pin the new
layout in place:
...
- [ ] **Cluster A** — `SettingsPage.test.tsx` +
  `SettingsPage_render.test.tsx` +
  `SettingsPage_destructive_confirm.test.tsx`; main file
  ≤ 500 lines; every original `it` block still runs; vitest
  count holds.
- [ ] **Cluster B** — `test_runner.py` + `test_runner_lifecycle.py`;
  both ≤ 500 lines; pytest count holds.
- [ ] **Cluster C** — `test_dat_basic.py` + `test_dat_security.py`
  + `test_dat_validation.py` (3-way split; 2-way fallback only if
  the < 100-line threshold from § Cluster C scope fires); all
  ≤ 300 lines; pytest count holds.

### Whole-pass acceptance

- [ ] All three file-size targets met (`wc -l` gate).
- [ ] No test deleted: `pytest --collect-only` returns identical
...
- [ ] All five backend gates green
  ...
  src`).
- [ ] Frontend gates green (`pnpm test && pnpm lint && pnpm tsc
  --noEmit`).
- [ ] Coverage gates hold: `copy/` ≥85%, `parser/` ≥90%, frontend
  ≥70%, overall ≥85%.
- [ ] `/audit` returns clean (the closing audit re-runs the
...
- [ ] CHANGELOG `[Unreleased]` updated; ROADMAP DS05 ticked ✅;
  three sub-items (1034/1035/1036) marked ✅ with reference to
  DS05; journal at `docs/journal/DS05.md`.
- [ ] Tagged `DS05-complete` (annotated); push (repo is PUBLIC).
