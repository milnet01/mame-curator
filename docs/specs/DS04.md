...
- No test file over the **hard cap** of 500 lines after the sweep. `test_runner.py` (528) and `test_fp01_fixes.py` (478, edge) are the at-risk files; T1.7 + T2 helper hoisting should bring `test_runner.py` under cap, and T2.9–T2.11 deletions should bring `test_fp01_fixes.py` further under. The deferred-to-1034/1035 splits are the safety net if either is still >500 post-T2.
